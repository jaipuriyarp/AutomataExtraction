...
def plot_line_graph_from_cumSum(df_list):
    ...
    # # Create a line plot for the constant values
    # plt.plot(df_line_graph[time_col], df_line_graph[y_col], marker='o', linestyle='-')
    #
    # # Customize the plot (optional)
    # plt.title('Constant Line Graph')
    ...
    # # Show the plot
    # plt.show()
def get_df_from_presentationTable_for_freq(df: pd.DataFrame, limitByTime:int, limitBySeq:int):
    time_col = 'Time(s)'
    ...
    # # ax.plot(df_per_sec[time_col], df_per_sec[y_col], label='Random testing',
    # #                   color='blue', linestyle='-')
    #
    # # fine_grained_ticks = np.arange(0, df_per_sec[y_col].max() + 1, 500)  # Adjust the step size as needed
    # # fine_grained_labels = [str(round(t, 1)) for t in fine_grained_ticks]
    #
    # # Set the x-axis tick positions and labels
    # # ax.set_yticks(fine_grained_ticks)
    # # ax.set_yticklabels(fine_grained_labels, rotation=45)
    #
    # # fine_grained_ticks = np.arange(0, df_per_sec[time_col].max() + 1, 1)  # Adjust the step size as needed
    # # fine_grained_labels = [str(round(t, 1)) for t in fine_grained_ticks]
    #
    # # Set the x-axis tick positions and labels
    # # ax.set_xticks(fine_grained_ticks)
    # # ax.set_xticklabels(fine_grained_labels, rotation=45)
    #
    # # print(df_per_sec)
    # # plt.show()
    # # Calculate the time intervals
    # interval_starts = [i * w for i in range(n)]
    # interval_ends = [(i + 1) * w for i in range(n)]
    #
    # # print(result_df)
    # ex_sums = []
    # ex_sums_cum = []
    # ex_sums_double = []
    # s = 0
    # # Iterate through the intervals and calculate the sum of 'ex' values
    # for start, end in zip(interval_starts, interval_ends):
    #     ex_sum = result_df[(result_df[time_col] >= start) & (result_df[time_col] < end)][y_col].sum()
    #     s += ex_sum
    #     ex_sums.append(ex_sum)
    #     ex_sums_cum.append(s)
    #     ex_sums_double.append(s)
    #     ex_sums_double.append(s)
    #
    # df_B = pd.DataFrame({time_col: [f'{start}-{end}' for start, end in zip(interval_starts, interval_ends)],
    #                      y_col: ex_sums,
    #                      'cumulative' : ex_sums_cum})
    #
    # # print(df_B)
    #
    # # # Plot a histogram of the 'ex sum' column
    # # ax = df_B.plot(kind='bar', x=time_col, y=y_col, edgecolor='black')
    # #
    # # # Customize the plot (optional)
    # # plt.title('Histogram of Ex Sum')
    # # plt.xlabel('Time Interval')
    # # plt.ylabel('Ex Sum')
    # #
    # # # Label the x-axis with 'time interval' values
    # # ax.set_xticklabels(df_B[time_col], rotation=0)
    # #
    # # # Show the plot
    # # plt.show()
    #
    # # print(interval_starts, interval_ends)
    #
    # # Extract 'time interval' and 'ex sum' columns
    # df_line_graph = pd.DataFrame()
    # time_intervals = sorted(interval_starts + [x-1 for x in interval_ends])
    # df_line_graph[time_col] = time_intervals
    # df_line_graph[y_col] = ex_sums_double

    # return df_line_graph
    return df_per_sec
...
def main():
    ...
    parser.add_argument('--file_suffix', type=str, default="_", help='Add suffix for file name.')
    ...
    for lang in range(1,9):
        if lang == 8:
            file_suffix = ""
        file1 = "lang" + str(lang) + "_presentationTable" + file_suffix + ".csv"
        file2 = "lang" + str(lang) + "_presentationTable_rSampling" + file_suffix + ".csv"
        ...
        if type == 0:
            df1_presentation = get_df_from_presentationTable(df1, limitByTime=None, limitBySeq=1500)
            df2_presentation = get_df_from_presentationTable(df2, limitByTime=None, limitBySeq=1500)
            df_list.append(df1_presentation)
            df_list.append(df2_presentation)

            # plot_overview_graph_from_df_list(df_list)
            plot_dataframes_in_subplots(df_list)

        elif type == 1:
            df1_presentation = get_df_from_presentationTable_for_freq(df1, limitByTime=400, limitBySeq=None)
            df2_presentation = get_df_from_presentationTable_for_freq(df2, limitByTime=400, limitBySeq=None)
            df_list.append(df1_presentation)
            df_list.append(df2_presentation)
